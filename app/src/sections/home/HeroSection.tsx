...
export default function HeroSection() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden">
      ...
      {/* Logo spotlight — makes transparent logo visible on any hero image */}
      <div
        className="absolute top-0 left-0 w-[400px] h-[200px] pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse at 30% 40%, rgba(255,252,237,0.5) 0%, rgba(255,252,237,0.2) 30%, transparent 70%)',
        }}
      />
      ...
      <div className="relative z-10 max-w-container mx-auto px-4 sm:px-6 lg:px-10 w-full pt-32 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-center lg:text-left">
            <h1
              ref={headingRef}
              className="font-heading text-3xl sm:text-4xl lg:text-5xl xl:text-6xl text-white leading-[1.1] tracking-[0.08em] uppercase opacity-0 font-light"
            ...
            <div ref={ctaRef} className="flex flex-col sm:flex-row items-center gap-4 mt-8 justify-center lg:justify-start opacity-0">
              <Link
                to="/course"
                className="inline-flex items-center gap-2 bg-yellow-500 text-green-950 px-7 py-3.5 rounded-md font-body font-medium text-xs uppercase tracking-[0.15em] hover:bg-yellow-400 transition-all hover:shadow-lg hover:-translate-y-0.5"
              >
                Explore Programs
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 border-2 border-white/80 text-white px-7 py-3.5 rounded-md font-body font-medium text-xs uppercase tracking-[0.15em] hover:bg-white/10 transition-all"
              >
                Book Consultation
              </Link>
            </div>
          ...
          {/* Right: Featured Course Card — HIDDEN (can be restored later) */}
          {false && (
